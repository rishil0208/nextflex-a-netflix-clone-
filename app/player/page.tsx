    ...
    return (
        <div className="bg-black text-white min-h-screen flex items-center justify-center relative">
            ...
            <div className="w-full h-screen relative">
                {/* Trailer Video or Placeholder */}
                {movie.trailerUrl ? (
                    <iframe
                        className="w-full h-full"
                        src={`${movie.trailerUrl}?autoplay=${isPlaying ? 1 : 0}&controls=1&modestbranding=1&rel=0`}
                        title={movie.title}
                    ...
                    >
                        ...
                        <div className="relative z-10 text-center">
                            <span className="text-6xl text-gray-400 mb-4 block">No Trailer Available</span>
                            <p className="text-xl text-gray-500">Full movie playback coming soon</p>
                        </div>
                    </div>
                )}
                ...
                <div className="absolute top-0 left-0 right-0 p-6 bg-gradient-to-b from-black/80 to-transparent z-20">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {movie.imdbRating > 0 && (
                                <span className="text-sm text-green-400">⭐ {movie.imdbRating}/10</span>
                            )}
                        </div>
                    </div>
                </div>

                {/* Movie Info Overlay (when not playing) */}
                {!isPlaying && movie.trailerUrl && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center z-10">
                        <div className="text-center max-w-2xl px-8">
                            ...
                            >
                                <span className="material-symbols-outlined text-6xl text-white">play_arrow</span>
                            </button>
